...
const AccountLedgerModal = ({ account, transactions, onClose }: { account: Account, transactions: Transaction[], onClose: () => void }) => {
  const { adjustBalance } = useAccounts(); // Get the new function
  const Icon = ICON_MAP[account.icon] || FlaskConical;
  ...
  const handleSaveBalance = async () => {
    const newBal = parseFloat(editBalanceVal);
    ...
    setEditLoading(true);
    try {
        await adjustBalance(account.id, newBal, editReason);
        setIsEditing(false);
        setEditReason('');
    } catch (error: any) {
      ...
      <div className="glass w-full max-w-2xl rounded-[2rem] md:rounded-[2.5rem] shadow-2xl relative animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh] overflow-hidden border border-white/10">
        ...
        <div className="p-5 md:p-8 pb-4 border-b border-white/5 bg-slate-900/50 shrink-0">
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 mt-6">
              <div className="p-3 md:p-4 bg-slate-900 rounded-xl md:rounded-2xl border border-white/5 flex flex-col justify-center relative group">
                 ...
                 {isEditing ? (
                    <div className="mt-2 space-y-2 animate-in fade-in">
                        <input 
                            ...
                            className="w-full bg-black border border-white/20 rounded-lg px-2 py-1 text-lg font-bold text-white focus:border-purple-500 outline-none"
                            autoFocus
                        />
                        <input 
                            type="text" 
                            placeholder="Reason (Req)" 
                            value={editReason} 
                            onChange={(e) => setEditReason(e.target.value)}
                            className="w-full bg-black border border-white/20 rounded-lg px-2 py-1 text-xs text-white focus:border-purple-500 outline-none"
                        />
                        <div className="flex gap-2">
                            <button onClick={handleSaveBalance} disabled={editLoading} className="flex-1 bg-purple-500 text-white text-xs font-bold py-1 rounded">
                                {editLoading ? '...' : 'Save'}
                            </button>
                            <button onClick={() => setIsEditing(false)} className="px-2 bg-slate-700 text-white text-xs font-bold py-1 rounded">Cancel</button>
                        </div>
                    </div>
                 ) : (
                    <div className="flex items-center justify-between">
                        <p className="text-xl md:text-2xl font-black text-white mt-0 md:mt-1">${account.balance.toLocaleString()}</p>
                        <button onClick={() => setIsEditing(true)} className="p-2 text-slate-600 hover:text-white transition-colors opacity-0 group-hover:opacity-100">
                            <Pencil size={14} />
                        </button>
                    </div>
                 )}
